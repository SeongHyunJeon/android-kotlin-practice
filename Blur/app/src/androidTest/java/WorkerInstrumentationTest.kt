...
class WorkerInstrumentationTest {
    ...
    private val data = workDataOf(KEY_IMAGE_URI to "android.resource://com.example.bluromatic/drawable/android_cupcake")
    ...
    fun blurWorker_doWork_resultSuccessReturnsUri() {
        ...
        runBlocking {
            ...
            assertTrue(uri?.startsWith("file:///data/user/0/com.example.bluromatic/files/blur_filter_outputs/blur-filter-output-") ?: false)
        }
    }
    ...
    fun saveImageToFileWorker_doWork_resultSuccessReturnsUrl() {
        ...
        runBlocking {
            ...
            assertTrue(result is ListenableWorker.Result.Success)
            assertTrue(result.outputData.keyValueMap.containsKey(KEY_IMAGE_URI))
            assertTrue(uri?.startsWith("content://media/external/images/media/") ?: false)
        }
    }
}